...
if !exists('g:rails_no_abbreviations') && type(get(g:, 'rails_no_abbreviations', {})) == type(0)
  call s:error('Use rails_no_abbreviations not rails_no_abbreviations to disable abbreviations')
...
function! s:BufAbbreviations()
  ...
  if !exists('g:rails_no_abbreviations')
    ...
    for pairs in
          \ items(type(get(g:, 'rails_no_abbreviations', 0)) == type({}) ? g:rails_no_abbreviations : {})
...
function! s:Abbrev(bang,...) abort
  if !exists("b:rails_no_abbreviations")
    let b:rails_no_abbreviations = {}
  endif
  if a:0 > 3 || (a:bang && (a:0 != 1))
    return s:error("Rabbrev: invalid arguments")
  endif
  if a:0 == 0
    for key in sort(keys(b:rails_no_abbreviations))
      echo key . join(b:rails_no_abbreviations[key],"\t")
    endfor
    return
  endif
  let lhs = a:1
  let root = s:sub(lhs,'%(::|\(|\[)$','')
  if a:bang
    if has_key(b:rails_no_abbreviations,root)
      call remove(b:rails_no_abbreviations,root)
    endif
  ...
  if lhs =~ '($'
    let b:rails_no_abbreviations[root] = ["(", rhs . (a:0 > 2 ? "\t".a:3 : "")]
    if a:0 > 2
  ...
  let b:rails_no_abbreviations[root] = [matchstr(lhs,'\W*$'),rhs]
endfunction
